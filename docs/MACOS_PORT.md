...
| **sfizz (optional SFZ)** | `audio-core/src/sfizz_sys.rs` dlopen, cfg-gated names: Linux `libsfizz.so.1`/`libsfizz.so`, macOS `libsfizz.1.dylib`/`libsfizz.dylib`, plus a bootstrap-installed absolute path tried first on macOS | **Done, automated** | sfztools' own [GitHub release](https://github.com/sfztools/sfizz/releases) (`sfizz-<ver>-macos.tar.gz`) is **x86_64-only** (their CI still targets `macos-11`, from before Apple-Silicon runners existed) — a native arm64 process can't dlopen it. `.github/workflows/build-sfizz-macos.yml` builds polyclav's own arm64 `libsfizz.dylib` from sfizz's vendored submodules on a real `macos-15` runner and publishes it as a polyclav release asset; `polyclav bootstrap` downloads it to `~/.local/share/polyclav/lib/libsfizz.dylib` automatically (`internal/bootstrap/sfizz.go`) — no Homebrew, no manual tarball wrangling. Optional backend either way; degrades gracefully if absent. |
...
| `sfizz` | 1.2.3 | ❌ (no formula, not needed) | Not a build dep — dlopen'd at runtime. `polyclav bootstrap` installs polyclav's own arm64 build automatically; see the coupling table above and `docs/INSTALL.md`. |
